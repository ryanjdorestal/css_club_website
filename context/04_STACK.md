...
| Components | **Hand-written to match `assets/refs/jj_inspo/`** (spec cards, stat tiles, ticker, numbered headers, corner brackets). shadcn/ui primitives allowed only for OS forms/tables/dialogs, restyled with the tokens. No component registries, no MCPs. | Ryan (T18): "get as close as you can to the typography and components of the jjay inspo folder." |
