...
for (const [name, route] of [
  ...
  if (name === "os-inheritance") {
    await p.locator("text=Adopt the new site").first().click();
    await p.waitForTimeout(1200);
    await p.screenshot({ path: `../../qa/loops/run8/os-inheritance-record-1440.png` });
    console.log(
      "panel:",
      await p.locator("aside").count(),
      "buttons:",
      await p
        .getByRole("button")
        .allTextContents()
        .then((a) => a.filter((t) => /edit/i.test(t))),
    );
    await p.getByRole("button", { name: /EDIT_RECORD/i }).click();
