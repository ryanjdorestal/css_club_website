...
const lum = (c) =>
  c.map((v) => (v <= 0.03928 ? v / 12.92 : ((v + 0.055) / 1.055) ** 2.4))
    .reduce((a, v, i) => a + v * [0.2126, 0.7152, 0.0722][i], 0);
const ratio = (a, b) => {
  const [l1, l2] = [lum(hex(a)), lum(hex(b))].sort((x, y) => y - x);
  return (l1 + 0.05) / (l2 + 0.05);
};
const T = { navy900:"#0C183C", navy600:"#1E4664", navy500:"#305484", ink:"#F4F7FB",
  muted:"#9DB0C4", teal:"#6ED2E6", red:"#CE4A4A", green:"#40A33F", blue:"#1E80F0", light:"#F5F7FA" };
const FG = { redFg:"#EAA3A3", greenFg:"#6FC46E", blueFg:"#7FB4F7", muted:"#A2B4C8" };
const pairs = [
