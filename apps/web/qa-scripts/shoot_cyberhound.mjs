...
console.log("mode:", await p.evaluate(() => document.querySelector("[data-cyberhound]")?.getAttribute("data-cyberhound")), "canvas:", await p.evaluate(() => !!document.querySelector("#cyberhound-slot canvas")));
await p.screenshot({ path: `${base}/${tag}-cyber-hero-3d.png` });
await p.mouse.move(1180, 420); await p.waitForTimeout(700);
await p.screenshot({ path: `${base}/${tag}-cyber-hero-3d-hover.png`, clip: { x: 880, y: 140, width: 560, height: 560 } });
await p.mouse.move(200, 500); await p.waitForTimeout(900);
await p.screenshot({ path: `${base}/${tag}-cyber-hero-3d-follow-left.png`, clip: { x: 880, y: 140, width: 560, height: 560 } });
await p.close();
const m = await b.newPage({ viewport: { width: 390, height: 844 } });
await m.goto("http://localhost:5173/cyberhounds", { waitUntil: "networkidle" }); await m.waitForTimeout(1500);
