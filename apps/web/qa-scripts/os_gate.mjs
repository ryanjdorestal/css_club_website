...
const base = "../../qa/loops/run8"; mkdirSync(base, { recursive: true });
...
// 1. anonymous
{
  ...
  await p.goto(`${BASE}/os/projects`, { waitUntil: "networkidle" }); await p.waitForTimeout(800);
...
// 2. guest = signed in but not on the roster
{
  ...
  const p = await ctx.newPage();
  await p.goto(`${BASE}/os`, { waitUntil: "networkidle" }); await p.waitForTimeout(800);
...
// 3. LOCAL_DEV admin via the picker
{
  const ctx = await b.newContext({ viewport: { width: 1440, height: 900 } });
  const p = await ctx.newPage();
  await p.goto(`${BASE}/os/login?next=%2Fos%2Fboard`, { waitUntil: "networkidle" }); await p.waitForTimeout(500);
  await p.locator('[data-testid="local-admin"]').click(); await p.waitForTimeout(1200);
...
// 4. public nav shows the button; signed-in nav shows the name
{
  ...
  await p.click('button[aria-label="Open menu"]'); await p.waitForTimeout(600);
