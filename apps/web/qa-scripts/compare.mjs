...
for (const item of plan) {
  const cells = [];
  for (const [img, crop] of [
    [`../../${item.refImage}`, item.refCrop],
    [`${base}/${item.ourShot}`, item.ourCrop],
  ]) {
    if (!existsSync(img)) {
      cells.push(
        await sharp({ create: { width: CW, height: 200, channels: 3, background: "#333" } })
          .png()
          .toBuffer(),
      );
      continue;
  ...
  const noteSvg = Buffer.from(
    `<svg width="${NOTEW}" height="${h}"><rect width="100%" height="100%" fill="#0C183C"/>${lines
      .slice(0, Math.floor((h - 20) / 16))
      .map((l, i) => `<text x="12" y="${24 + i * 16}" fill="#F4F7FB" font-family="monospace" font-size="12">${esc(l)}</text>`)
      .join("")}</svg>`,
  );
