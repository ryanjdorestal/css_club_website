...
export function HomePartners() {
  return (
    <>
      <CubeAnchor id="partners" kf={{ x: 0.93, y: 0.22, scale: 0.26, face: "threeQuarter", glow: brand.palette.teal }}>
        <Band
          tone="light-2"
          accent="teal"
          index="07 — PROGRAMS & PARTNERS"
          sigil={<Sg.PlusMark size={16} />}
          code="SLOTS"
          rail="07 · PARTNERS · 01010000 · SLOTS"
        >
          <RevealGroup className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              ...
              <RevealItem key={p2.title}>
                <a
                  ...
                  className="group flex flex-col justify-between border border-(--tone-line) p-4 min-h-[110px] hover:border-(--accent) transition-colors"
                >
            ...
            <RevealItem>
              <SlotCard n="04" label="Open" action="partner ↗" />
            </RevealItem>
            <RevealItem>
              <SlotCard n="05" label="Open" />
            </RevealItem>
          </RevealGroup>
        </Band>
      </CubeAnchor>
