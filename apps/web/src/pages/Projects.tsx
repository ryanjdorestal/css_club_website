    ...
    <main>
      ...
      {featured && (
        <Band
          tone="tinted"
          accent="green"
          index="01 — FEATURED · SPEC SHEET"
          sigil={<Sg.Terminal size={16} />}
          code="FEATURED"
          rail="01 · PROJECTS · 01010000 · FEATURED"
        >
              ...
              <IndexList
                rows={[
                  { index: "1", bracket: true, title: "Submit", dek: "Title, kind, summary, link — two minutes", sigil: <Sg.Terminal size={14} /> },
                  {
                    ...
                    bracket: true,
                    title: "Board review",
                    dek: "Fit + a working link; a written note if changes are needed",
                    sigil: <Sg.Eye size={14} />,
                  },
      ...
      <Band
        tone="light"
        accent="green"
        index="03 — PROJECTS · RESEARCH · TOOLS"
        sigil={<Sg.Lambda size={16} />}
        code="DISPLAY"
        rail="03 · PROJECTS · 01010010 · OPEN SLOTS"
      >
        {projects.length ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            <p className="text-[16px] leading-relaxed max-w-[58ch] mb-8" style={{ color: "var(--tone-muted)" }}>
              Nothing published yet — which is the pitch. Capstones, class projects and research get the same review and the same byline as apps. Three slots
              are open.
            </p>
...
function SubmitBand() {
  ...
  return (
    <Band
      ...
      accent="green"
      index="04 — SUBMIT"
      sigil={<Sg.ArrowSq size={16} />}
      code="INTAKE"
      rail="04 · FORM · 01010011 · TIER-1 SAFE"
      id="submit"
    >
          ...
          <Reveal>
            <div className="border border-(--tone-line) p-8">
              ...
              <p className="text-sm leading-relaxed" style={{ color: "var(--tone-muted)" }}>
                {result.stored === "api"
                  ? "The board reviews every submission in the OS; you'll hear back by email."
          ...
          <form onSubmit={onSubmit} className="border border-(--tone-line)">
            {[
              { k: "_TITLE *", el: <input required name="title" className={field} placeholder="Campus Room Finder" /> },
              {
                k: "KIND *",
                el: (
                  <select required name="kind" className={`${field} bg-transparent`}>
                    {KINDS.map((k) => (
              ...
              {
                k: "SUMMARY *",
                el: <textarea required name="summary" rows={4} className={field} placeholder="What it does + how it helps John Jay students" />,
              },
            ].map((row) => (
            ...
            <div className="px-5 py-4">
              <Button type="submit" variant="ghost" disabled={busy}>
                {busy ? "submitting" : ">_submit_project"}
              </Button>
            </div>
          </form>
        )}
