...
export function CSSKufic({ size = 24, className = "", style }: { size?: number; className?: string; style?: React.CSSProperties }) {
  ...
  const cells: [number, number][] = [...C, ...S.map(([x, y]) => [x + 6, y] as [number, number]), ...S.map(([x, y]) => [x + 12, y] as [number, number])];
  return (
...
export const Flag = (p: P) => (
  <svg {...base(p)}>
    <path d="M6 21 V3 M6 4 H18 L15 8 L18 12 H6" />
  </svg>
);
...
export const Node = (p: P) => (
  <svg {...base(p)}>
    ...
    <path d="M12 3 V9 M12 15 V21 M3 12 H9 M15 12 H21" />
...
export const Shield = (p: P) => (
  <svg {...base(p)}>
    ...
    <path d="M8 11 L11 14 L16 8" />
  </svg>
);
export const BracketSigil = (p: P) => (
  <svg {...base(p)}>
    <path d="M9 3 H4 V21 H9 M15 3 H20 V21 H15" />
  </svg>
);
export const Crosshair = (p: P) => (
  <svg {...base(p)}>
    <circle cx="12" cy="12" r="7" />
    <path d="M12 1 V6 M12 18 V23 M1 12 H6 M18 12 H23" />
    <rect x="11" y="11" width="2" height="2" fill="currentColor" stroke="none" />
  </svg>
);
export const Chevrons = (p: P) => (
  <svg {...base(p)}>
    <path d="M4 6 L10 12 L4 18 M10 6 L16 12 L10 18 M16 6 L22 12 L16 18" />
  </svg>
);
export const Star4 = (p: P) => (
  <svg {...base(p)}>
    <path d="M12 2 L14 10 L22 12 L14 14 L12 22 L10 14 L2 12 L10 10 Z" fill="currentColor" stroke="none" />
  </svg>
);
export const Lambda = (p: P) => (
  <svg {...base(p)}>
    <path d="M6 21 L12 4 L18 21 M9.5 14 H14.5" />
  </svg>
);
/** Pixel eye (T03). */
