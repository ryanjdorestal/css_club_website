...
export function PosterCard({
  ...
  const inner = (
    <div className={`group relative bg-navy-900 text-ink overflow-hidden aspect-[4/5] ${className}`}>
      ...
      {index && (
        <span className="absolute top-5 left-5 t-micro raise text-(--accent-fg)">
          {"//"}
          {index}
        </span>
      )}
      ...
      {mark && (
        <span aria-hidden className="absolute top-11 right-5">
          {mark}
        </span>
      )}
      <span className="absolute bottom-4 left-0 right-0 t-wide uppercase leading-[0.85] text-[clamp(30px,3.6vw,58px)]">
        <span className="relative block px-4">
          <span
            aria-hidden
            className="t-outline absolute left-4 top-0 opacity-0 group-hover:opacity-60 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-200 whitespace-pre-line"
          >
            {word}
          </span>
