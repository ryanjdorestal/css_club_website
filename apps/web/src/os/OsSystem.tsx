...
export default function OsSystem() {
  ...
  useEffect(() => {
    void osFetch<{ checks: Check[] }>("/api/os/status").then((r) => setChecks(r.ok ? r.data.checks : [{ state: "offline", label: "API", detail: r.error ?? "unreachable", fix: "make dev (README)" }]));
  }, []);
              ...
              {ownEdit.map((a, i) => (
                <div key={a.name} className="grid md:grid-cols-[180px_1fr_1fr_140px] gap-2 px-3 py-2 items-center">
                  ...
                  {(["owner_email", "second_owner_email", "last_verified"] as const).map((k) => (
                    <input key={k} aria-label={`${a.name} ${k.replace(/_/g, " ")}`} value={a[k] ?? ""} placeholder={k.replace(/_/g, " ")} onChange={(e) => setOwnEdit(ownEdit.map((x, j) => (j === i ? { ...x, [k]: e.target.value } : x)))} className="bg-transparent border-b border-line px-1 py-1 font-mono text-[12px] text-ink focus:border-teal outline-none" />
                  ))}
                </div>
              ))}
