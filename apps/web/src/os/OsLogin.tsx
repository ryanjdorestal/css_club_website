...
const input =
  "w-full bg-transparent border-0 border-b border-line px-1 py-2.5 font-mono text-sm text-ink placeholder:text-muted/40 focus:border-teal outline-none";
      ...
      <div>
        ...
        <h1 className="t-h1 !text-[clamp(44px,7vw,96px)] !leading-[0.92] mt-4">
          <span className="block">
            <Stencil bars={[0.5]} barColor="var(--color-navy-900)">
              BOARD
            </Stencil>
          </span>
          <span className="block">
            <Outline>ACCESS.</Outline>
          </span>
        </h1>
        <p className="text-[16px] leading-relaxed text-muted max-w-[52ch] mt-6">
          For the current {brand.name} board only. CSS OS is where the board writes {MODULES} — everything the public site shows, plus what the next board needs
          to know. Access is granted by the roster: if you're on the board and can't get in, the president or webmaster adds your school email on{" "}
        ...
        <p className="t-micro opacity-40 mt-8">
          ...
          {mode === "unconfigured" && (
            <div>
              ...
              <p className="text-[13px] text-muted mt-2 leading-relaxed">
                This deployment has no Supabase Auth env vars, so there is no way to sign in yet. The public site works regardless. SETUP.md turns this on in 8
                steps.
              </p>
            </div>
          )}
          {mode === "local" && (
            <div>
              ...
              <p className="t-micro opacity-50 mt-3">
                Email login turns on with the Supabase env vars (SETUP.md). In this dev build use the LOCAL_DEV panel below.
              </p>
            </div>
          )}
        </div>
            ...
            <div className="flex gap-2 mt-3">
              {(["officer", "admin"] as Role[]).map((r) => (
                <button
                  ...
                  className="mono-label px-4 py-2.5 border border-line text-muted hover:text-ink hover:border-teal transition-colors cursor-pointer"
                >
                  → {r}
                </button>
              ))}
